...
void CNewMenuHook::UnsubClassMenu(HWND hWnd)
{
  ...
  SetLastError(0);
  if(!SetWindowLong(hWnd, GWLP_WNDPROC, (DWORD)(DWORD_PTR)oldWndProc))
  {
    ShowLastError();
  }
...
BOOL CNewMenuHook::CheckSubclassing(HWND hWnd, BOOL bSpecialWnd)
{
  ...
  if(pData==NULL)
  {
    ...
    oldWndProc = (WNDPROC)(LONG_PTR)GetWindowLong(hWnd, GWLP_WNDPROC);
    if (oldWndProc != NULL && GetProp(hWnd, _OldMenuProc) == NULL)
    {
      ...
      if ((WNDPROC)GetProp(hWnd, _OldMenuProc) == oldWndProc)
      {
        ...
        { 
          ...
          SetLastError(0);
          if(!SetWindowLong(hWnd, GWLP_WNDPROC,(DWORD)(DWORD_PTR)SubClassMenu))
          {
            ShowLastError();
          }
          bDoNewSubclass = TRUE;
...
public:
  CNewMenuTempHandler(HWND hWnd)
  {
    ...
    m_oldWndProc = (WNDPROC)(LONG_PTR)SetWindowLong(hWnd, GWLP_WNDPROC,(DWORD)(DWORD_PTR)TempSubclass);
  }

  ~CNewMenuTempHandler()
  {
    SetWindowLong(m_hWnd, GWLP_WNDPROC,(DWORD)(DWORD_PTR)m_oldWndProc);
