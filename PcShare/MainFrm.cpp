...
LRESULT CMainFrame::OnAddClient(WPARAM wParam,LPARAM lParam)
{
	...
	if(pClientListView->GetListCtrl().GetItemCount() >
		...
		return FALSE;
	char m_Text[256] = { 0 };
	sprintf(m_Text, "count:%d",pClientListView->GetListCtrl().GetItemCount());
	...
	sprintf(m_Text,"客户【%s - %s】已经连接 ID:%d  count:%d",
		pItem->m_Title,	pItem->m_SysInfo.m_PcName,pItem->m_SysInfo.ID,pClientListView->GetListCtrl().GetItemCount());
	ShowMyText(m_Text , FALSE);
